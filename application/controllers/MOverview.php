...
class MOverview extends MY_Controller {
		...
		foreach($file_data as $cadkey => $cadvalue){
			if(!empty($cost_data)){
				...
				if($cost_data->unit_cost != '1'){
					...
					$amount = $volume * $cost_data->density * $cost_data->material_cost;
				}
	...
	public function box_calc($part_length, $part_breadth, $part_height, $part_quantity){
		...
		$box_a_length_divide = floor($calc_defualt->box_a_length/$part_length);
		$box_a_breadth_divide = floor($calc_defualt->box_a_breadth/$part_breadth);
		$box_a_height_divide = floor($calc_defualt->box_a_height/$part_height);
		$box_a = (($box_a_length_divide) * ($box_a_breadth_divide) * ($box_a_height_divide));
		if($box_a >= 1){
			$box_a_float_quantity = ($part_quantity/$box_a);
		}else{
			$box_a_float_quantity = 0;
		}
		$box_a_int_quantity = $this->get_quantity($box_a_float_quantity);
		array_push($quantity_array, $box_a_int_quantity);
		
		$box_b_length_divide = floor($calc_defualt->box_b_length/$part_length);
		$box_b_breadth_divide = floor($calc_defualt->box_b_breadth/$part_breadth);
		$box_b_height_divide = floor($calc_defualt->box_b_height/$part_height);
		$box_b = (($box_b_length_divide) * ($box_b_breadth_divide) * ($box_b_height_divide));
		if($box_b >= 1){
			$box_b_float_quantity = ($part_quantity/$box_b);
		}else{
			$box_b_float_quantity = 0;
		}
		$box_b_int_quantity = $this->get_quantity($box_b_float_quantity);
		array_push($quantity_array, $box_b_int_quantity);
		
		$box_c_length_divide = floor($calc_defualt->box_c_length/$part_length);
		$box_c_breadth_divide = floor($calc_defualt->box_c_breadth/$part_breadth);
		$box_c_height_divide = floor($calc_defualt->box_c_height/$part_height);
		$box_c = (($box_c_length_divide) * ($box_c_breadth_divide) * ($box_c_height_divide));
		if($box_c >= 1){
			$box_c_float_quantity = ($part_quantity/$box_c);
		}else{
			$box_c_float_quantity = 0;
		}
		$box_c_int_quantity = $this->get_quantity($box_c_float_quantity);
		array_push($quantity_array, $box_c_int_quantity);
		
		$box_d_length_divide = floor($calc_defualt->box_d_length/$part_length);
		$box_d_breadth_divide = floor($calc_defualt->box_d_breadth/$part_breadth);
		$box_d_height_divide = floor($calc_defualt->box_d_height/$part_height);
		$box_d = (($box_d_length_divide) * ($box_d_breadth_divide) * ($box_d_height_divide));
		if($box_d >= 1){
			$box_d_float_quantity = ($part_quantity/$box_d);
		}else{
			$box_d_float_quantity = 0;
		}
		$box_d_int_quantity = $this->get_quantity($box_d_float_quantity);
		array_push($quantity_array, $box_d_int_quantity);
		
		foreach ($box_properties as $key => $value) {
			if($key == 'box_a'){
				$box_properties[$key]['value'] = $box_a;
				$box_properties[$key]['float_quantity'] = $box_a_float_quantity;
				$box_properties[$key]['int_quantity'] = $box_a_int_quantity;
			}
			if($key == 'box_b'){
				$box_properties[$key]['value'] = $box_b;
				$box_properties[$key]['float_quantity'] = $box_b_float_quantity;
				$box_properties[$key]['int_quantity'] = $box_b_int_quantity;
			}
			if($key == 'box_c'){
				$box_properties[$key]['value'] = $box_c;
				$box_properties[$key]['float_quantity'] = $box_c_float_quantity;
				$box_properties[$key]['int_quantity'] = $box_c_int_quantity;
			}
			if($key == 'box_d'){
				$box_properties[$key]['value'] = $box_d;
				$box_properties[$key]['float_quantity'] = $box_d_float_quantity;
				$box_properties[$key]['int_quantity'] = $box_d_int_quantity;
			}
		}
	...
	public function weight_calc($part_volume, $part_quantity, $part_density){
		...
		foreach ($box_properties as $key => $value) {
			if($least_quantity == $value['int_quantity'] && empty($volumetric) && empty($gross)){
				$length_name = $value['name'].'_length';
				$breadth_name = $value['name'].'_length';
				$height_name = $value['name'].'_length';
				$box_volume = (($calc_defualt->$length_name) *($calc_defualt->$breadth_name) *($calc_defualt->$height_name));
		...
		// $actual_cost = 	'&#8377; '.$inr_cost.' - &#36; '.$doller_cost;
	...
	public function get_quantity($quantity){
		$decimal = substr(strrchr(number_format($quantity, 4), "."), 1);
	...
	public function get_weight($weight){
		$decimal = substr(strrchr(number_format($weight, 4), "."), 1);
		$weight = str_replace("," , "" , $weight);
		if($decimal >= 1 && $decimal <= 9999){
