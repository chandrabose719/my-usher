<div id="about-part" class="px-5 wrapper-body-padding">    
    <section id="about">
        <div class="container">
            <div class="row">
            	<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">	
                    <div class="about-content">
                    	<h1>About Us</h1>
                    </div>    
            	</div>
            </div>
        </div>
    </section>
    <section id="mission">
        <div class="container">
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                    <div class="mission-content">
                        <h3>Our Mission</h3>
                        <p>We focus on setting up a medium that anchors upon providing you with the apt technology for your needs to ensure that the time between the initiation and the completion of your manufacturing process is cut down.</p>
                        <p>Our goal is to help you optimize 3D designing, materials and manufacturing processes. The right key for your lock.</p>
                    </div>    
                </div>
            </div>
        </div>
    </section>
    <div class="container">
        <hr>    
    </div>
    <section id="we-do">
        <div class="container">
            <h3>We do this by</h3>
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                    <div class="we-do-content">
                        <h4>Instant Quote Generation</h4>
                        <p>With a 3D design file, instant quoting gives you transparency in what the manufacturing cost will be, & allows you flexibility in altering 3D design, process & material selection.</p>
                    </div>
                </div>
            </div>        
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                    <div class="we-do-content">
                        <h4>Easy Order Placement</h4>
                        <p>Once all the manufacturing details are complete, you can place your order directly on 3D Usher.</p>    
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                    <div class="we-do-content">
                        <h4>Quick Turnaround Time</h4>
                        <p>You'll get email updates on your order status in real time, while we complete the order and run quality control checks on your product before sending it to you.</p>
                    </div>    
                </div>
            </div>
        </div>
    </section>
    <div class="container">
        <hr>    
    </div>
    <section id="team">
        <div class="container">
            <h3>Team</h3>
            <div class="row">
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="https://www.linkedin.com/in/gopalkrishna/" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Gopal_3DUsher.jpg'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>    
                        <h4>Gopal Krishna</h4>
                        <p>CEO</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="https://www.linkedin.com/in/fznmehdi/" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Faizan_3DUsher.jpg'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>
                        <h4>Faizan Mehdi</h4>
                        <p>COO</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="https://www.linkedin.com/in/thea-vinereanu-278498134/" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Thea_3DUsher.jpg'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>
                        <h4>Thea Vinereanu</h4>
                        <p>CMO</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <!-- <div class="img-custom"> -->
                            <!-- <a href="" target="_blank"> -->
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Poorna_3DUsher.jpg'); ?>">
                                <!-- <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div> -->
                            <!-- </a>     -->
                        <!-- </div> -->
                        <h4>Poorna Chander Uppala</h4>
                        <p>Advisor/Early Investor</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="http://www.linkedin.com/in/dhiraj-nayak" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Dhiraj_3DUsher.jpg'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>
                        <h4>Dhiraj Nayak</h4>
                        <p>Product Lead</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="https://www.linkedin.com/in/meghan-casey-1bb070156/" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/MeghanCasey_3DUsher.jpg'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>
                        <h4>Meghan Casey</h4>
                        <p>Marketing Associate</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="https://www.linkedin.com/in/yashwant-rahul-reddy-02298398/" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Yashwant_3DUsher.png'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>
                        <h4>Yashwant Reddy</h4>
                        <p>Marketing Strategist</p>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <div class="img-custom">
                            <a href="https://www.linkedin.com/in/chandrabose719/" target="_blank">
                                <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/Chandrabose_3DUsher.jpg'); ?>">
                                <div class="team-profile">    
                                    <i class="fab fa-linkedin-in"></i>
                                </div>
                            </a>    
                        </div>
                        <h4>Chandrabose</h4>
                        <p>Web Development</p>
                    </div>    
                </div>    
            </div>
            <div class="container">
                <hr>    
            </div>
            <h3>Advisors</h3>
            <div class="row">
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/NasirAli_3DUsher.jpg'); ?>">
                        <h4>Nasir Ali</h4>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/ChuckStormon_3DUsher.jpg'); ?>">
                        <h4>Chuck Stormon</h4>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/JamesShomar_3DUsher.jpg'); ?>">
                        <h4>James Shomar</h4>
                    </div>    
                </div>
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/JamesJoseph_3DUsher.jpg'); ?>">
                        <h4>Jim Joseph</h4>
                    </div>    
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/RandyWolken_3DUsher.jpg'); ?>">
                        <h4>Randy Wolken</h4>
                    </div>    
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/LauraMiller_3DUsher.jpg'); ?>">
                        <h4>Laura Miller</h4>
                    </div>    
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid rounded-circle" src="<?= base_url('assets/images/about/DayaprasadKulkarni_3DUsher.jpg'); ?>">
                        <h4>Dr. Dayaprasad Kulkarni</h4>
                    </div>    
                </div>   
            </div>
            <div class="container">
                <hr>    
            </div>
            <h3>We are also supported by</h3>
            <div class="row">
                <div class="col-xl-4 offset-xl-2 col-lg-4 offset-lg-2 col-md-4 offset-md-2 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid" src="<?= base_url('assets/images/about/Start-Fast.jpg'); ?>">
                    </div>    
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid" src="<?= base_url('assets/images/about/Madden.jpg'); ?>">
                    </div>    
                </div>
                <div class="col-xl-4 offset-xl-2 col-lg-4 offset-lg-2 col-md-4 offset-md-2 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid" src="<?= base_url('assets/images/about/Tech-Garden.jpg'); ?>">
                    </div>    
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-xs-12">
                    <div class="team-content">
                        <img class="img-fluid" src="<?= base_url('assets/images/about/Arc-Ventures.jpg'); ?>">
                    </div>    
                </div>
            </div>    
        </div>
    </section>
    <div class="container">
        <hr>    
    </div>
    <section id="our-location">
        <div class="container">
            <h3>Our Promise</h3>
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                    <div class="our-location-content">
                        <p>We ensure you that your product is manufactured within the best possible time frame and the quality of the product is well examined before it is delivered to you.</p>
                    </div>
                </div>
            </div>
            <div class="container">
                <hr>    
            </div>
            <h3>Reach us at the following office locations</h3>
            <div class="row">
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-12"> 
                    <div class="our-location-address">
                        <h4><a href="https://goo.gl/maps/6SuHcvFiV8F2" target="_blank">
                            <i class="fas fa-directions"></i>
                        </a></h4>
                        <h4>New York</h4>
                        <p>Mitchell Hall Room 102,</p>
                        <p>1421 Salt Springs Rd,</p>
                        <p>Syracuse, NY 13214</p>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-12"> 
                    <div class="our-location-address">
                        <h4><a href="https://goo.gl/maps/UDszWby8xrL2" target="_blank">
                            <i class="fas fa-directions"></i>
                        </a></h4>
                        <h4>Hyderabad</h4>
                        <p>Plot no 23,24, 2nd Floor,</p>
                        <p>Shaktipuram Industrial Estate,</p>
                        <p>Prashantinagar, Kukatpally,</p>
                        <p>Hyderabad, Telangana - 500072</p>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>